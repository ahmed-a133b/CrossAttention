...
class PlantDiseaseDataset(Dataset):
    ...
    def __init__(
        ...
        # Load available classes from classes.txt if it exists, otherwise use prompts keys
        classes_file = os.path.join(root_dir, 'classes.txt')
        if os.path.exists(classes_file):
            with open(classes_file, 'r') as f:
                all_available_classes = [line.strip() for line in f if line.strip()]
            print(f"Loaded {len(all_available_classes)} classes from classes.txt")
        else:
            all_available_classes = list(self.prompts_data.keys())
            print(f"No classes.txt found, using {len(all_available_classes)} classes from prompts file")
        
        # Get all available classes or use selected subset
        if selected_classes is None:
            self.class_names = sorted(all_available_classes)
        else:
            # Check which selected classes are available
            self.class_names = [cls for cls in selected_classes if cls in all_available_classes]
            if len(self.class_names) != len(selected_classes):
                missing = set(selected_classes) - set(all_available_classes)
                print(f"Warning: Classes not found in available classes: {missing}")
        
        # Verify that we have prompts for all selected classes
        missing_prompts = [cls for cls in self.class_names if cls not in self.prompts_data]
        if missing_prompts:
            print(f"Warning: No prompts found for classes: {missing_prompts}")
            # Filter out classes without prompts
            self.class_names = [cls for cls in self.class_names if cls in self.prompts_data]
    ...
    def _load_samples(self, max_samples_per_class: Optional[int] = None) -> List[Tuple[str, int]]:
        ...
        # For direct class folder structure, the images_dir is simply the root_dir
        # Each class is a folder directly in the root directory
        images_dir = self.root_dir
        
        if not os.path.exists(images_dir):
            raise FileNotFoundError(f"Dataset root directory not found: {images_dir}")
        
        # Get all subdirectories (these should be class folders)
        subdirs = [d for d in os.listdir(images_dir) 
                  if os.path.isdir(os.path.join(images_dir, d))]
        
        # Filter out non-class directories (system files, etc.)
        excluded_dirs = {'__pycache__', '.git', 'checkpoints', 'logs', 'results', '.ipynb_checkpoints'}
        class_dirs = [d for d in subdirs if d not in excluded_dirs]
        
        # Check how many directories match our expected classes
        matching_dirs = []
        for class_dir in class_dirs:
            # Direct match with class names from prompts
            if class_dir in self.class_names:
                matching_dirs.append(class_dir)
            # Check for underscore vs space variations
            elif class_dir.replace('_', ' ') in self.class_names:
                matching_dirs.append(class_dir)
            elif class_dir.replace(' ', '_') in self.class_names:
                matching_dirs.append(class_dir)
        
        print(f"Found {len(matching_dirs)} matching class directories in {images_dir}")
        
        if not matching_dirs:
            print(f"Available directories: {class_dirs[:10]}")  # Show first 10
            print(f"Expected classes: {self.class_names[:5]}")   # Show first 5
            raise FileNotFoundError(f"No matching class directories found in {images_dir}")
        
        print(f"Using images directory: {images_dir}")
        
        # Check if split file exists (optional)
        possible_split_files = [
            os.path.join(self.root_dir, 'trainval.txt'),
            os.path.join(self.root_dir, 'split.txt'),
            'trainval.txt'
        ]
    ...
    def _load_from_directories(self, images_dir: str, max_samples_per_class: Optional[int]) -> List[Tuple[str, int]]:
        ...
        # Get all subdirectories in the images directory
        available_dirs = [d for d in os.listdir(images_dir) 
                         if os.path.isdir(os.path.join(images_dir, d))]
        
        for class_name in self.class_names:
            # Try to find matching directory (handle name variations)
            class_dir = None
            possible_names = [
            ...
            for possible_name in possible_names:
                potential_dir = os.path.join(images_dir, possible_name)
            ...
            if class_dir is None:
                ...
                print(f"Available directories: {available_dirs[:10]}...")  # Show first 10
                continue
            
            class_label = self.class_to_idx[class_name]
            image_files = [f for f in os.listdir(class_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff'))]
...
def create_data_loaders(
    ...
    # Find prompts file in various locations
    possible_prompts_files = [
        os.path.join(config.dataset_root, 'plantwild_prompts.json'),
        'plantwild_prompts.json',  # Current directory
        '/content/plantwild_prompts.json'  # Colab location
    ]
    ...
    if prompts_file is None:
        raise FileNotFoundError(f"plantwild_prompts.json not found. Tried: {possible_prompts_files}")
...
if __name__ == "__main__":
    ...
    config = ModelConfig(
        dataset_root='./data',  # Path to folder containing class directories
        selected_classes=['apple black rot', 'apple scab', 'tomato leaf mold'],
        batch_size=8,
    ...
    if not os.path.exists(prompts_file):
        print("Error: plantwild_prompts.json not found in current directory")
        exit(1)
